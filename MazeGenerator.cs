...
public class MazeGenerator : MonoBehaviour
{
    ...
    public Transform walls, Tile_regular;
    public class Cell
    {
        public bool Visited;
                ...
                for (int p = 0; p < cellArea; p++)
                {
                    if (maze[y * row + x].VisitedS)
                    {
                        for (int i = 0; i < wallThickness; i++)  // Destroy all the walls on that direction
                        {
                            ...
                            Vector3 v3 = new Vector3(x * 2 + (wallThickness - 1) * x, 0, y * 2 + 1 + i + (wallThickness - 1) * y);
                            Destroy(posNobj[v3]);
                    ...
                    if (maze[y * row + x].VisitedE)
                    {
                        for (int i = 0; i < wallThickness; i++)
                        {
                            Vector3 v3_1 = new Vector3(x * 2 + 1 + i + (wallThickness - 1) * x, 0, y * 2 + (wallThickness - 1) * y);
